...
def write_novel(
    ...
    for idx, plan in enumerate(plans, start=1):
        ...
        for j, scene in enumerate(scenes, 1):
            if on_stage:
                on_stage(f"第{idx}章 写作场景 {j}/{len(scenes)}")
        ...
        if settings.style_check:
            if on_stage:
                on_stage(f"第{idx}章 风格检查")
            style_dev = check_style_deviation(provider, profile, chapter_text)
        if settings.plot_check:
            if on_stage:
                on_stage(f"第{idx}章 剧情一致性检查")
            plot_cons = check_plot_consistency(provider, novel.plot_state, chapter_text, idx)
        if (
